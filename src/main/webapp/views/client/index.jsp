...
<tiles:insertDefinition name='main' >
    ...
    <tiles:putAttribute name='body'>
        ...
        <div class='content-wrapper'><!-- Content Wrapper. Contains page content -->
            <section class='content-header'><!-- Content Header (Page header) -->
                <h1><spring:message code='default.button.list.label' text='List'/> Client</h1>
                <ul class='top-links'>
    ...
    <th class='center bold'><spring:message code='default.button.action.label' text='Action'/></th> 
            ...
            <c:if test='${not empty clients}'>
                <c:forEach items='${clients}' var='client'  varStatus='loopStatus'>
                    <tr class='${loopStatus.index % 2 == 0 ? 'odd' : 'even'}'>
                        <td><c:out value='${client.code}'/></td>
                        <td><c:url var='picFile' value='/client/getPhotoTumb/${client.picFile}'/>
<img alt='${client.picFile}' src='${picFile}'/></td>
                        <td><c:out value='${client.fullName}'/></td>
                        <td><c:out value='${client.gender}'/></td>
                        <td><fmt:formatDate value='${client.dob}' type='date' pattern='dd/MM/yyyy'/></td>
                        <td><c:out value='${client.email}'/></td>
                        <td><c:out value='${client.address}'/></td>

                           <td class='center action'>
                            <ul class='top-links'>
                            <sec:access url='/client/show'>
                                <li>
                                <a class='btn btn-block btn-info btn-xs' href='${pageContext.request.contextPath}/client/show/${client.id}'>
                                    <i class='fa fa-info-circle'></i> <spring:message code='show.link.label'/>
                                </a>
                                </li>
                            </sec:access>
                            <sec:access url='/client/edit'>
                                <li>
                                <a class='btn btn-block btn-primary btn-xs' href='${pageContext.request.contextPath}/client/edit/${client.id}'>
                                    <i class='fa fa-edit'></i> <spring:message code='edit.link.label'/>
                                </a>
                                </li>
                            </sec:access>
                            <sec:access url='/client/copy'>
                                <li>
                                <a class='btn btn-block btn-warning btn-xs' href='${pageContext.request.contextPath}/client/copy/${client.id}'>
